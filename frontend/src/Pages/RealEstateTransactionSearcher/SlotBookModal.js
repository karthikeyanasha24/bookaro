...
const SlotBookModal = ({
  ...
  const submit = () => {
    if (form.isRequest) {
      if (!form.requestNote)
        return toast.error(t("transactionSearcher.provideRequestNote"));
      save(form);
    } else {
      if (!form.slot) return toast.error(t("transactionSearcher.selectSlot"));
      save(form);
    }
  };
          ...
          <DialogTitle className="p-3">
            <div className="react-custom">
              <p className="border-b text-[#389D93] text-[18px] text-center pb-5 mt-3">
                {title || t("transactionSearcher.chooseVisitSlots")}
              </p>
              ...
              <div className="flex border-t p-2 justify-between">
                ...
                >
                  {form.isRequest ? t("transactionSearcher.request") : t("transactionSearcher.book")}
                </button>
              </div>
            </div>
          </DialogTitle>
