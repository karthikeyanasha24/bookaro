...
const getAuthRequestConfig = (params = {}) => ({
...
const getAuthFormConfig = () => {
  const token = localStorage.getItem("token");
  if (token) {
    return { headers: { ...imageConfig.headers, Authorization: `Bearer ${token}` } };
  }
  return imageConfig;
};

const handleError = (err, hideError, statusCode) => {
  let message = "";
  const isUnauthorized =
    (err && err.error && err.error.code === 401) || statusCode === 401;
...
class ApiClient {
  static post(url1, payload, params = {}, base = "", hideError = false) {
    ...
    return new Promise((fulfill, reject) => {
      axios
        .post(url, JSON.stringify(payload), { headers: getAuthHeaders(), params })
        .then(response => fulfill(response?.data))
        .catch(error => {
          loader(false);
          if (error?.response) {
            const eres = error.response;
            handleError(eres.data, hideError, eres.status);
  ...
  static put(url1, payload, base = "") {
    ...
    return new Promise((fulfill, reject) => {
      axios
        .put(url, JSON.stringify(payload), { headers: getAuthHeaders() })
        .then(response => fulfill(response?.data))
        .catch(error => {
          loader(false);
          if (error?.response) {
            const eres = error.response;
            handleError(eres.data, false, eres.status);
            fulfill(eres.data);
          } else {
            toast.error("Network Error");
            reject(error);
  ...
  static patch(url1, payload, base = "", hideError = false) {
    ...
    return new Promise((fulfill, reject) => {
      axios
        .patch(url, JSON.stringify(payload), { headers: getAuthHeaders() })
        .then(response => fulfill(response?.data))
        .catch(error => {
          loader(false);
          if (error?.response) {
  ...
  static get(url1, params = {}, base = "", hideError = "") {
    ...
    return new Promise((fulfill, reject) => {
      axios
        .get(url, getAuthRequestConfig(params))
        .then(response => fulfill(response?.data))
        .catch(error => {
          loader(false);
          if (error?.response) {
  ...
  static delete(url1, params = {}, base = "") {
    ...
    return new Promise((fulfill, reject) => {
      axios
        .delete(url, getAuthRequestConfig(params))
        .then(response => fulfill(response?.data))
        .catch(error => {
          loader(false);
          if (error?.response) {
  ...
  static postFormData(url, params) {
    ...
    return new Promise((fulfill, reject) => {
      ...
      axios
        .post(fullUrl, body, getAuthFormConfig())
        .then(response => fulfill(response?.data))
        .catch(error => {
          loader(false);
          if (error?.response) {
  ...
  static multiImageUpload(url, files, params = {}, key = "file") {
    ...
    return new Promise((fulfill, reject) => {
      axios
        .post(fullUrl, body, { headers: getAuthFormConfig().headers, params })
        .then(response => fulfill(response?.data))
        .catch(error => {
          loader(false);
          if (error?.response) {
