...
const BuildingPermit = () => {
  ...
  const navigateToList = () => {
    ...
    if (form.latitude)
      query.push(`latitude=${encodeURIComponent(form.latitude)}`);
    if (form.longitude)
      query.push(`longitude=${encodeURIComponent(form.longitude)}`);
    if (form.type) query.push(`type=${encodeURIComponent(form.type)}`);
    const str = query.length > 0 ? `?${query.join("&")}` : "";
    navigate(`/building-permit-list${str}`);
  };
  return (
    <PageLayout>
      <div>
            ...
            <div className="border border-[#8492A6] rounded-[12px] bg-white p-5  lg:max-w-[800px] md:max-w-[600px] w-[80%] mx-auto absolute md:-bottom-10 -bottom-[120px] left-set ">
              <div className="flex flex-wrap justify-between items-end md:flex-row flex-col">
                <div className="md:w-[40%] w-[100%]">
                  <div className="flex items-center w-full custom-loc relatitudeive">
                    <div className="relative w-full">
                      <MdOutlineLocationOn className="me-2 absolute z-[1] left-[10px] top-1/2 transform-all -translate-y-1/2" />
                ...
                <div className="md:w-[36%] w-[100%] md:my-0 my-5 bg-[#f0f0f0]">
                  <SelectDropdown
