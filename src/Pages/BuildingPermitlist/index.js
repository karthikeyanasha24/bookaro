...
const BuildingPermitlist = () => {
  ...
  console.log(allfilters,"allfiletr")
  ...
  const getData = (f = {}) => {
    ...
    if (activePlan?.activePlan?.[0]?.otherDetails?.browseBuildingPermits?.key == "") {
      return
    }
  ...
  // Inside your component
  const debouncedGetFilterResult = useRef(
    debounce((currentIndFilter) => {
      let dto = { ...filters, ...currentIndFilter };

      if (user?._id) {
        dto = {
          ...dto,
          loggedInUser: user?._id,
          maxDistance: currentIndFilter?.maxDistance,
        };
      }
  ...
  // Update the getFilterResult function to use debounce
  const getFilterResult = useCallback(() => {
    debouncedGetFilterResult(indFilter);
  }, [debouncedGetFilterResult, indFilter]);

  useEffect(() => {
    getFilterResult();
  }, [indFilter, getFilterResult]);

  // Cleanup on unmount
  useEffect(() => {
    return () => {
      debouncedGetFilterResult.cancel();
    };
  }, [debouncedGetFilterResult]);
