...
const Step1 = ({ step1, setActiveTabIndex, formData, setFormData, id }) => {
  ...
  const [newplanModal, newsetplanModal] = useState(false)
  useEffect(() => {
    ...
    if (activePlan?.activePlan?.[0]?.otherDetails.createPropProfileSaleRentDirectory.key === "custom" && activePlan?.activePlan?.[0]?.otherDetails.createPropProfileSaleRentDirectory.value <= activePlan?.activePlan?.[0]?.propertyCount) {
      setplanModal(true)
    }
  }, []);
  ...
  return (
    <div className="flex justify-between flex-col h-full relative">
      {!id && <PropertyCheck setplanModal={setplanModal} planModal={planModal} />}
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                <input
                  ...
                  // disabled={activePlan?.activePlan?.[0]?.otherDetails?.accessToOffMarketProps?.key == "custom" && (activePlan?.activePlan?.[0]?.otherDetails?.accessToOffMarketProps?.value <= activePlan?.activePlan?.[0]?.offMarketPropertyCount)}
                  onChange={(e) => {
                    if (activePlan?.activePlan?.[0]?.otherDetails?.accessToOffMarketProps?.key == "custom" && (activePlan?.activePlan?.[0]?.otherDetails?.accessToOffMarketProps?.value <= activePlan?.activePlan?.[0]?.offMarketPropertyCount)) {
                      newsetplanModal(true)
                    } else {
        ...
        <div className="px-5">
          ...
          <div className="inline-flex gap-3 items-center bg-[#fff] p-2 rounded-md border border-[#976DD0] mt-4">
            <input
              ...
              // disabled={activePlan?.activePlan?.[0]?.accessToOffMarketProps?.key == "custom" && activePlan?.activePlan?.[0]?.accessToOffMarketProps?.value <= activePlan?.activePlan?.[0]?.offMarketPropertyCount}
              onChange={(e) => {
                if (activePlan?.activePlan?.[0]?.otherDetails?.accessToOffMarketProps?.key == "custom" && (activePlan?.activePlan?.[0]?.otherDetails?.accessToOffMarketProps?.value <= activePlan?.activePlan?.[0]?.offMarketPropertyCount)) {
