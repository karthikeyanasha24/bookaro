  ...
  const columns = [
    {
      ...
      sort: true,
      render: (row) => {
        return <span className="capitalize">{row?.reportTo?.fullName}</span>;
      },
    },
    {
      key: "reportTo",
      ...
      sort: true,
      render: (row) => {
        return <span>{row?.addedBy?.fullName}</span>;
      },
    },
    {
      key: "reason",
      ...
      sort: true,
      render: (row) => {
        return <span>{stringSeprator(row?.reason, 50)}</span>;
      },
    },
    {
      key: "status",
          ...
          <>
            <div className="flex items-center justify-start gap-1.5">
              <Tooltip placement="top" title="View">
                <span onClick={(e) => view(itm.id)} className="border cursor-pointer  hover:opacity-70 rounded-[35px] bg-[#00988e1c] w-10 h-10 !text-primary flex items-center justify-center text-lg text-[#222]">
                  <PiEyeLight />
                </span>
              </Tooltip>
              {itm?.status === "pending" && (
                <>
                  <Tooltip placement="top" title="Accept">
                    <span
                      onClick={(e) => acceptRequest(itm, "accepted")}
                  ...
                  <Tooltip placement="top" title="Reject">
                    <span
                      onClick={(e) => acceptRequest(itm, "rejected")}
                      className="border cursor-pointer  hover:opacity-70 rounded-[35px] bg-[#00988e1c] w-10 h-10 !text-primary flex items-center justify-center text-lg text-[#222]"
                    >
                      <RxCrossCircled />
                    </span>
    ...
    <Layout>
      ...
      <div className="shadow-box w-full bg-white rounded-lg mt-6">
        <div className="flex p-4 items-center flex-wrap gap-2">
          ...
          <div className="flex gap-2 md:ml-auto ml-0">
            <SelectDropdown
              id="statusDropdown"
              displayValue="name"
