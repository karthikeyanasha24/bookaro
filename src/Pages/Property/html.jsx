...
const Html = ({
  ...
  const columns = [
    ...
    },
    {
      ...
      sort: true,
      render: (row) => {
        return <span className="capitalize">{row?.surface ? `${row.surface} m²` : "--"}</span>
      },
    },
    {
      key: "city",
      ...
      sort: true,
      render: (row) => {
        return <span className="capitalize">{row?.city || row?.address || "--"}</span>
      },
    },
    {
      key: "zipcode",
      ...
      sort: true,
      render: (row) => {
        return <span className="capitalize">{row?.zipcode || "--"}</span>
      },
    },
    {
      key: "owner",
    ...
    },
    {
      ...
      sort: true,
      render: (row) => {
        return <span className="capitalize">{row?.createdAt ? new Date(row.createdAt).toLocaleDateString() : "--"}</span>
      },
    },
            ...
            <div className="flex items-center justify-start gap-1.5">
              ...
              {isAllow(`read${shared.check}`) && (
                <Tooltip placement="top" title="Admin View">
                  <a onClick={(e) => adminView(itm?.id)} className="border cursor-pointer hover:opacity-70 rounded-[35px] bg-[#976dd021] w-10 h-10 flex items-center justify-center text-lg text-[#976DD0]">
