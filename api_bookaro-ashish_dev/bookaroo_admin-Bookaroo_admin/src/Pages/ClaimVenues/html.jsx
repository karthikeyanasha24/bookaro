...
const Html = ({
  ...
  const columns = [
    {
      key: "claim_venue_data",
      name: "Venue Name",
      sort: true,
      render: (itm) => {
        const data = itm?.claim_venue_data
        return <Tooltip placement="top" title="View">
          <span className="capitalize cursor-pointer" onClick={(e) => view(itm.id)}>{data?.venue_name}</span>
        </Tooltip>
      },
    },
    {
      ...
      render: (row) => {
        return <div className="w-32">
          <span className={`text-sm !px-3 h-[30px] w-[100px] flex items-center justify-center border border-[#EBEBEB] !rounded capitalize ${row?.request_status == "rejected" ? " bg-red-600 text-white" : row?.request_status == "verified" ? "bg-green-600 text-white" : "bg-yellow-600 text-white"}`}>
            {row?.request_status}
          </span>
        </div>
      },
