  ...
  const columns = [
    {
      ...
      sort: true,
      render: (row) => {
        return <span className="capitalize">{row?.venue_name}</span>
      },
    },
    {
      key: "status",
    ...
    },
    {
      ...
      render: (row) => {
        return (
          <>
            <div className="w-32" onClick={() => statusChange(row)}>
              <span className={`cursor-pointer text-sm !px-3 h-[30px] w-[100px] flex items-center justify-center border border-[#EBEBEB] !rounded capitalize ${row?.doc_status == "rejected" ? " bg-red-600 text-white" : row?.doc_status == "verified" ? "bg-green-600 text-white" : "bg-yellow-600 text-white"}`}>
            ...
            <div className="flex items-center justify-start gap-1.5">
              ...
              {itm?.status === "active" &&
                <Tooltip placement="top" title="Add Post">
                  <span onClick={() => addPost(itm?.id)} className="border cursor-pointer  hover:opacity-70 rounded-[35px] bg-[#00988e1c] w-10 h-10 !text-primary flex items-center justify-center text-lg text-[#222]">
        ...
        <div className="flex p-4 items-center flex-wrap gap-2">
          ...
          <div className="flex gap-2 ml-auto">
            ...
            <SelectDropdown
              ...
              result={(e) => { handleFilter(e.value, 'docStatus') }}
