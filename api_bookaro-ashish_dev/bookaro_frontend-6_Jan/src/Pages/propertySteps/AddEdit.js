...
const AddEdit = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    maxLeads: "",
    email: user?.email,
    username: user.role === "agency"
      ? `${capLetter(user.companyName)}`
      : `${capLetter(user.firstName)}.${capLetter(user.lastName?.slice(0, 1))} - Private Owner`,
      ...
      ApiClient.get(shared.detailApi, { id }).then((res) => {
        if (res.success) {
          ...
          setFormData({
            ...data,
            id: data?.id || data?._id,
            school1: { schoolId: { value: filterSchool1data?.schoolId, label: filterSchool1data?.EstablishmentName }, type: filterSchool1data?.type },
            school2: { schoolId: { value: filterSchool2data?.schoolId, label: filterSchool2data?.EstablishmentName }, type: filterSchool2data?.type },
            school3: { schoolId: { value: filterSchool3data?.schoolId, label: filterSchool3data?.EstablishmentName }, type: filterSchool3data?.type },
            school4: { schoolId: { value: filterSchool4data?.schoolId, label: filterSchool4data?.EstablishmentName }, type: filterSchool4data?.type },
  ...
  const checkFillData = (index) => {
    if (index == 0) {
      return true
    } else if (index == 1 && formData?.type) {
      return true
    } else if (index == 2 && (formData?.address)) {
      return true
    } else if (index == 3 && (!formData.surface=="" ||
      formData.bathroom>0 ||
      formData.rooms>0 ||
      formData.bedrooms>0 || formData.totalFloorBuilding>0 ||
      formData.toilets>0 || formData.propertyFloor>0 ||
      formData.livingRoom>0 || formData.building>0 ||
      formData.propertyFloor>0 || formData.situation?.length>0 ||
      formData.cooking?.length>0 || formData.equipment?.length>0 ||
      formData.outside?.length>0 || formData.serviceAccessibility?.length>0||
      formData.ancilliary?.length>0 || formData.environment?.length>0 || formData.leisure?.length>0 ||
      formData.investment?.length>0
    )) {
      return true
    } else if (index == 4 && (formData?.energyConsumption ||
      formData?.energy_efficient ||
      formData?.emissions ||
      formData?.emission_efficient ||
      ...
      formData?.contact)) {
      return true
    } else if (index == 5 && (formData.images?.length>0)) {
      return true
    } else if (index == 6 && (formData?.propertyTitle || formData?.content)) {
      return true
    } else if (index == 7 && (formData?.school1 || formData?.school2 || formData?.school3 || formData?.school4 )) {
      return true
    } else if (index == 8 && (formData?.propertyCharges ||
      ...
      formData.rateLeads ||
      formData.maxLeads ||
      formData.propertyAgencyFees ||
      formData.propertyAgencyFees ||
      formData.propertyInventory
    )) {
      return true
    } else if (index == 9 && (
      formData?.real_estate_market ||
      formData?.sale_my_property
    )) {
      return true
    } else if (index == 10 && (formData.revenue_detail)) {
      return true
    }else if (index == 11 && (formData.Expenses)) {
      return true
    }else if (index == 12 && (formData.renovation_work)) {
      return true
    }else if (index == 13 && (formData.rating)) {
      return true
    }else{
      return false
    }
                      ...
                      <div className="fixed inset-0 overflow-y-auto">
                        <div className="flex min-h-full items-center justify-start h-full text-center">
                          ...
                          >
                            ...
                            ${((checkFillData(index) && index != activeTabIndex) || index<activeTabIndex)
                    ...
                    {steps.map((label, index) => {
                      if (
                        ...
                        (formData?.propertyType === "directory" ||
                          (step1?.propertyType &&
                            step1.propertyType.toLowerCase() === "directory"))
                      )
                        return (
                          <Tab
                            key={index}
                            className={`hidden border border-[#976DD0] rounded-[50px] p-3 text-[#545757] text-center my-3 lg:mx-0 mx-1 md:w-full w-[160px] text-[14px] lg:text-[16px] cursor-pointer
                            ${((checkFillData(index) && index != activeTabIndex) || index<activeTabIndex)
                                ? "bg-[#c1a8e1] text-white border-[#c1a8e1]"
                                : index == activeTabIndex
                                  ? "bg-[#976DD0] text-white"
                      ...
                      return (
                        <Tab
                          key={index}
                          className={`border border-[#976DD0] rounded-[50px] p-3 text-[#545757] text-center my-3 lg:mx-0 mx-1 md:w-full w-[160px] text-[14px] lg:text-[16px] cursor-pointer
                          ${((checkFillData(index) && index != activeTabIndex) || index<activeTabIndex)
                              ? "bg-[#c1a8e1] text-white border-[#c1a8e1]"
                              : index == activeTabIndex
                                ? "bg-[#976DD0] text-white"
