...
async function listBuildingPermits({
  ...
  try {
    ...
    let findUser;
    if (loggedInUser) {
      findUser = await db.users.findById(loggedInUser)
        .populate('planId').select('otherDetails')

      if (findUser.planId?.otherDetails?.browseBuildingPermits?.key === "custom") {

        console.log("findUser.planId?.otherDetails?.browseBuildingPermits?.value", findUser.planId?.otherDetails?.browseBuildingPermits?.value)

        limit = Number(findUser.planId?.otherDetails?.browseBuildingPermits?.value) || 10;
      }
    ...
    if (loggedInUser && findUser.planId?.otherDetails?.browseBuildingPermits?.key === "custom") {
      totalCount = limit
    }


    return { total: totalCount, data: mappedResults, limit };
  } catch (error) {
