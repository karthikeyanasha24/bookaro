...
const Users = () => {
  ...
  const acceptRequest = (itm, value) => {
    Swal.fire({
      title: "Are you sure?",
      text: `Do you want to ${value==="accepted" ? "accept" : "reject"} the request to report account?`,
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#063688",
      cancelButtonColor: "#d33",
      ...
      if (result.isConfirmed) {
        loader(true);
        ApiClient.get(shared.statusApi, { status: value, id: itm.id }).then((res) => {
          if (res.success) {
            getData();
          }
          loader(false);
