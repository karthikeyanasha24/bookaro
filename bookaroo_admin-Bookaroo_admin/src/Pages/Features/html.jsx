...
import { stringSeprator } from "../../models/string.models";
  ...
  const columns = [
    {
      ...
      sort: true,
      render: (row) => {
        return <span className="capitalize">{stringSeprator(row?.name,50)}</span>;
      },
    },
    {
      key: "name",
      ...
      sort: true,
      render: (row) => {
        return <span className="capitalize">{row?.featureType}</span>;
      },
    },
    {
      key: "status",
          ...
          <>
            ...
            {isAllow(`edit${shared.check}`) ? (
              <Tooltip placement="top" title="Edit">
                <a onClick={(e) => edit(itm)} className="border cursor-pointer  hover:opacity-70 rounded-[35px] bg-[#00988e1c] w-10 h-10 !text-primary flex items-center justify-center text-lg text-[#222]">
                  <LiaEdit />
                </a>
              </Tooltip>
            ):(<></>)}
    ...
    <Layout>
      <div className="flex flex-wrap justify-between items-center gap-y-4">
        ...
        {isAllow(`add${shared.check}`) ? (
          <div className="bg-primary leading-10 mr-3 h-10 flex items-center shadow-btn px-6 hover:opacity-80 text-sm text-white rounded-lg gap-2 cursor-pointer" onClick={()=>handleAddFeatures()}>
      ...
      <div className="shadow-box w-full bg-white rounded-lg mt-6">
        <div className="flex p-4 items-center flex-wrap">
          <form className="flex items-center max-w-sm" onSubmit={(e) => { e.preventDefault(); filter(); }}>
            ...
            <button type="submit" className="p-2.5 m-2 text-sm font-medium text-white bg-blue-700 rounded-lg border border-[#976DD0] focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
              <svg
                className="w-4 h-4"
                aria-hidden="true"
      ...
      <AddEdit isOpen={isOpen} setIsOpen={setIsOpen} form={form} setForm={setForm} getFeaturesListing={getFeaturesListing} />
    </Layout>
  );
};
